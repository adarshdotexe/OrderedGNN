...
def get_trainer(params):
    ...
    if dataset_name in ['Chameleon', 'Squirrel']:
        dataset = WikipediaNetwork(root='datasets/datasets_pyg/', geom_gcn_preprocess=True, name=dataset_name, transform=T.Compose([T.ToUndirected(), T.AddSelfLoops(), T.NormalizeFeatures(), T.ToSparseTensor()]))
        data = dataset[0]
    ...
    if dataset_name in ['Cornell', 'Texas', 'Wisconsin']:
        dataset = WebKB(root='datasets/datasets_pyg/', name=dataset_name, transform=T.Compose([T.ToUndirected(), T.AddSelfLoops(), T.NormalizeFeatures(), T.ToSparseTensor()]))
        data = dataset[0]
        data.train_mask = dataset[0].train_mask[:,int(split)]
        data.val_mask = dataset[0].val_mask[:,int(split)]
        data.test_mask = dataset[0].test_mask[:,int(split)]
    ...
    if dataset_name in ['Actor']:
        dataset = Actor(root='datasets/datasets_pyg/Actor', transform=T.Compose([T.ToUndirected(), T.AddSelfLoops(), T.NormalizeFeatures(), T.ToSparseTensor()]))
        data = dataset[0]
        data.train_mask = dataset[0].train_mask[:,int(split)]
        data.val_mask = dataset[0].val_mask[:,int(split)]
        data.test_mask = dataset[0].test_mask[:,int(split)]
    ...
    if dataset_name in ['Cora_full','CiteSeer_full','PubMed_full']:
        dataset = Planetoid(root='datasets/datasets_pyg/', name='%s'%(dataset_name.split('_')[0]), split=dataset_name.split('_')[-1], transform=T.Compose([T.ToUndirected(), T.AddSelfLoops(), T.NormalizeFeatures(), T.ToSparseTensor()]))
        data = dataset[0]
        params['in_channel']=data.num_features
        params['out_channel']=dataset.num_classes

    if dataset_name in ['Cora_geom','CiteSeer_geom','PubMed_geom']:
        dataset = Planetoid(root='datasets/datasets_pyg/', name='%s'%(dataset_name.split('_')[0]), split='public', transform=T.Compose([T.ToUndirected(), T.AddSelfLoops(), T.NormalizeFeatures(), T.ToSparseTensor()]))
